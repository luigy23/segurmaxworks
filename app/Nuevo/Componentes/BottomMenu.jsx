  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg">
      <div className="flex justify-between items-center h-16 max-w-md mx-auto px-4">
        <MenuItem2 link={"/Nuevo/Categorias"} icon={<HouseIcon className="h-6 w-6 text-gray-600" />} label="Inicio" />
        ...
        <MenuItem2 link={"/Nuevo/"} icon={<HouseIcon className="h-6 w-6 text-gray-600" />} label="Inicio" />
...
const MenuItem2 = ({ icon, link, isCenter }) => (
  <Link href={link} className={`flex flex-col items-center ${isCenter ? 'transform translate-y-[-20px]' : ''}`}>
